...
export default function LoginPage() {
  ...
  const onSubmit = async (data: LoginFormValues) => {
    try {
      ...
      // Store the token (matches the interceptor we built earlier)
      if (typeof window !== 'undefined') {
        if (response.token) localStorage.setItem('logiq_token', response.token);
      }
    ...
    <div className="min-h-screen bg-[#0B0F19] flex items-center justify-center p-4">
      <div className="w-full max-w-md bg-[#111827] border border-slate-800 rounded-xl p-8 shadow-2xl">
        <div className="flex flex-col items-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-white tracking-wider">LOGIQ</h1>
          <p className="text-slate-400 text-sm mt-1">Intelligent Log Monitoring</p>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
          ...
          <button
            ...
            className="w-full bg-blue-600 hover:bg-blue-700 text-white font-medium py-2.5 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex justify-center items-center"
          >
